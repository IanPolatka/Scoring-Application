            ...
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="home_team_id" id="home_team_id" class="form-control" onChange="window.location.href=this.value">
                            ...
                            @foreach($teams as $team)
                              ...
                              <option value="/basketball-boys/{{$showcurrentyear[0]}}/{{ $team->school_name }}">{{ $team->school_name }}</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="home_team_id"class="form-control" onChange="window.location.href=this.value">
                            ...
                            @foreach($years as $year)
                              ...
                              <option value="/basketball-boys/{{ $year->year }}" @if ($showcurrentyear[0] == $year->year) selected @endif>
                                {{ $year->year }}
                              </option>
                ...
                @if (Auth::user())
                    ...
                    <div class="col-lg-2">
                        ...
                        <p><a class="pull-right btn btn-success" href="/basketball-boys/create">Create Game</a></p>
                        ...
                        <!-- @forelse ($basketball as $item)
                            ...
                            <li class="list-group-item">
                                ...
                                @if ( $item->away_team->logo )
                                    <img src="/images/team-logos/{{ $item->away_team->logo }}" style="height: 20px; width: 20px; border-radius: 3px;margin-right: 5px;">
                                @endif
                                <a href="/basketball-boys/{{ $showcurrentyear[0] }}/{{ $item->away_team->school_name }}">{{ $item->away_team->school_name }}</a> vs
                                @if ( $item->home_team->logo )
                                    <img src="/images/team-logos/{{ $item->home_team->logo }}" style="height: 20px; width: 20px; border-radius: 3px;margin: 0px 5px;"> 
                                @endif
                                <a href="/basketball-boys/{{ $showcurrentyear[0] }}/{{ $item->home_team->school_name }}">{{ $item->home_team->school_name }}</a>
                                @if ($item->tournament_title)
                                    <small>({{ $item->tournament_title }})</small>
                                @endif
                                @if (Auth::user())
